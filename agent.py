...
def build_system_prompt(
    ...
    base = f"""You are a helpful coding assistant.
...
async def run_agent(
    ...
    from permissions import DEFAULT_PATH_BASED, DEFAULT_AUTO_ALLOW_CWD, DEFAULT_AUTO_ALLOW
    ...
    console.print(Panel(
        f"[bold] A pedagogical Claude Code clone\n"
    ...
    session = (
        PromptSession(
            history=FileHistory(os.path.expanduser("~/.henri_history")),
    ...
    while True:
        try:
            ...
            if not user_input.strip():
                continue
            await agent.chat(expand_file_references(user_input, console=console))
        ...
        while True:
            ...
            async for event in self.provider.stream(
                ...
                if event.text:
                    self._cancel_pondering()
                    self._hide_status()
                    # self.console.print(event.text, end="") I don't care about streaming. 
            ...
            for i, call in enumerate(tool_calls):
                ...
                self._hide_status()
                try:
                    allowed = self.permissions.check(tool, call)
                except ToolLoopAborted:
                    # Every tool_use block still needs a matching result,
                    # including the calls we never got to.
                    for pending in tool_calls[i:]:
                        results.append(ToolResult(
                            tool_call_id=pending.id,
                            content="[not executed: user stopped the turn]",
                            is_error=True,
                        ))
                    aborted = True
                    break
